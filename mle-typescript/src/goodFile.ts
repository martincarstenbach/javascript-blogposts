 ...
 * @param {IPurchaseOrder} purchaseOrder - the PO to update
 * @param {string} lastUpdate - a string representation of a date (YYYY-MM-DDThh:mm:ss)
 * @returns {IPurchaseOrder} - the updated purchaseOrder
 */
function setLastUpdatedDate(
	...
	let verifiedLastUpdate = lastUpdate;
	if (verifiedLastUpdate === undefined || verifiedLastUpdate.length === 0) {
		verifiedLastUpdate = new Date().toISOString();
	}
	purchaseOrder.lastUpdate = verifiedLastUpdate;
 ...
 * lint using eslint.
 * @param {IPurchaseOrder} purchaseOrder - the PO to validate
 ...
 */
export function processPurchaseOrder(
	poNumber: IPurchaseOrder["PONumber"],
): void {
